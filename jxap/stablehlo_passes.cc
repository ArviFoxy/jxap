...
absl::StatusOr<std::string> RefineInputTypes(absl::string_view mlir_code,
  ...
  for (const auto& type_str : mlir_types) {
    mlir::Type parsed_type = mlir::parseType(type_str, &context);
    if (!parsed_type) {
      return absl::InvalidArgumentError(
          absl::StrCat("Failed to parse type ", type_str, ". Diagnostics:\n", diagnostic_output));
    }
  ...
  // Pass 1: refine program input shapes to be static.
  pm.addPass(mlir::stablehlo::createStablehloRefineArgumentsPass(parsed_types));
  // Pass 2: propagates static shapes across the program.
  pm.addPass(mlir::stablehlo::createStablehloRefineShapesPass());
  // Pass 3: replaces dynamic shape ops with static shape ops if possible.
  pm.addPass(mlir::stablehlo::createStablehloCanonicalizeDynamismPass());
  // Pass 4: simplify and propagate constants.
