...
absl::Status ErrorToStatus(PJRT_Error* error, const PJRT_Api* api) {
  ...
  return absl::Status(
    static_cast<absl::StatusCode>(code_args.code), 
    msg_args.message);
}
 ...
 public:
  static absl::StatusOr<std::unique_ptr<PJRTContext>> Create() {
    ...
    if (api->pjrt_api_version.major_version != PJRT_API_MAJOR ||
      ...
      LOG(ERROR) << "Warning: PJRT API version mismatch. Expected "
                << PJRT_API_MAJOR << "." << PJRT_API_MINOR
                << ", Got " << api->pjrt_api_version.major_version
...
class PJRTExecutable {
 ...
 public:
  static absl::StatusOr<std::unique_ptr<PJRTExecutable>> Load(std::string path, PJRTContext* ctx) {
    ...
    // compile_options is a serialized CompileOptionsProto. For default optimizations:
  ...
  void PrintStats(absl::string_view path, absl::string_view method) const {
    ...
    LOG(INFO) << "| Generated code size (MB) : " << stats_args.generated_code_size_in_bytes / (1024.0 * 1024.0);
    LOG(INFO) << "| Argument size       (MB) : " << stats_args.argument_size_in_bytes / (1024.0 * 1024.0);
    LOG(INFO) << "| Output size         (MB) : " << stats_args.output_size_in_bytes / (1024.0 * 1024.0);
    LOG(INFO) << "| Alias size          (MB) : " << stats_args.alias_size_in_bytes / (1024.0 * 1024.0);
    LOG(INFO) << "| Temp size           (MB): " << stats_args.temp_size_in_bytes / (1024.0 * 1024.0);
    LOG(INFO) << "| Host generated code size (MB): " << stats_args.host_generated_code_size_in_bytes / (1024.0 * 1024.0);
    LOG(INFO) << "| Host argument size       (MB): " << stats_args.host_argument_size_in_bytes / (1024.0 * 1024.0);
    LOG(INFO) << "| Host output size         (MB): " << stats_args.host_output_size_in_bytes / (1024.0 * 1024.0);
    LOG(INFO) << "| Host alias size          (MB): " << stats_args.host_alias_size_in_bytes / (1024.0 * 1024.0);
    LOG(INFO) << "| Host temp size           (MB): " << stats_args.host_temp_size_in_bytes / (1024.0 * 1024.0);
    LOG(INFO) << "--------------------------------------------------------------------------------";
  }
};
...
void AwaitAndDestroyEvent(PJRT_Event* event, const PJRT_Api* api, const std::string& event_name) {
    if (!event) return;
    ...
    // PJRT_Event_Await returns the error status *of the operation that the event is tracking*.
    PJRT_Error* operation_error = api->PJRT_Event_Await(&await_args);
    CheckError(operation_error, api, "Operation error from PJRT_Event_Await for " + event_name);
...
int main() {
    ...
    try {
        ...
        num_outputs_args.executable = underlying_executable;
        CheckError(api->PJRT_Executable_NumOutputs(&num_outputs_args), api, "PJRT_Executable_NumOutputs");
        size_t num_outputs_per_device = num_outputs_args.num_outputs;

        if (num_outputs_per_device != 1) { // This example specifically expects one output
            throw std::runtime_error("Example requires 1 output, but MLIR model has " + std::to_string(num_outputs_per_device));
        }
        std::cout << "Executable has " << num_outputs_per_device << " output(s) per device." << std::endl;

        // 5. Prepare input data
        std::vector<float> host_input_data = {10.0f, 20.0f, 30.0f, 40.0f};
        const int64_t input_dims[] = {static_cast<long long>(host_input_data.size())};
        size_t num_input_dims = 1;

        // 6. Create input buffer on device from host data
        PJRT_Client_BufferFromHostBuffer_Args bhfh_args;
        bhfh_args.struct_size = PJRT_Client_BufferFromHostBuffer_Args_STRUCT_SIZE;
        bhfh_args.extension_start = nullptr;
        bhfh_args.client = client;
        bhfh_args.data = host_input_data.data();
        ...
        bhfh_args.host_buffer_semantics = PJRT_HostBufferSemantics_kImmutableUntilTransferCompletes;
        bhfh_args.device = device;
        bhfh_args.memory = nullptr; // Use default memory for the device
        bhfh_args.device_layout = nullptr; // Use default layout
        // Output fields:
        bhfh_args.done_with_host_buffer = nullptr;
        bhfh_args.buffer = nullptr;

        CheckError(api->PJRT_Client_BufferFromHostBuffer(&bhfh_args), api, "PJRT_Client_BufferFromHostBuffer (input)");
        input_buffer = bhfh_args.buffer;
        ...
        std::cout << "Input buffer created on device and H2D transfer complete." << std::endl;

        // 7. Prepare for Execution
        PJRT_Buffer* const* single_arg_list = &input_buffer; // Array of input buffers for one execution
        PJRT_Buffer* const* const* all_devices_arg_lists = &single_arg_list; // List of arg lists (for one device)

        // Prepare space for output buffers: PJRT_LoadedExecutable_Execute will populate this.
        // For 1 device, 1 output:
        std::vector<PJRT_Buffer*> device_output_buffers_list(num_outputs_per_device); // e.g., {nullptr}
        ...
        execute_args.output_lists = output_lists_for_execute_arg; // API populates this
        execute_args.execute_device = device; // Explicitly specify single device for execution
        // Output field for completion event:
        PJRT_Event* execution_complete_event = nullptr;
        execute_args.device_complete_events = &execution_complete_event; // For single device execution

        // 8. Execute the model
        CheckError(api->PJRT_LoadedExecutable_Execute(&execute_args), api, "PJRT_LoadedExecutable_Execute");
        std::cout << "Execution launched." << std::endl;

        AwaitAndDestroyEvent(execution_complete_event, api, "model execution");
        std::cout << "Execution completed." << std::endl;

        output_buffer_from_execute = device_output_buffers_list[0]; // Retrieve the populated output buffer
        if (!output_buffer_from_execute) {
            throw std::runtime_error("Execute did not return an output buffer.");
        }
        ...
        CheckError(api->PJRT_Buffer_Dimensions(&out_dim_args), api, "PJRT_Buffer_Dimensions (output)");
        ...
        CheckError(api->PJRT_Buffer_ElementType(&out_type_args), api, "PJRT_Buffer_ElementType (output)");

        if (out_type_args.type != PJRT_Buffer_Type_F32) {
            throw std::runtime_error("Output buffer type is not F32 as expected.");
        }
        size_t num_output_elements = 1;
        std::cout << "Output buffer has " << out_dim_args.num_dims << " dimension(s): ";
        for (size_t i = 0; i < out_dim_args.num_dims; ++i) {
            num_output_elements *= out_dim_args.dims[i];
            std::cout << out_dim_args.dims[i] << " ";
        }
        std::cout << "(Total elements: " << num_output_elements << ")" << std::endl;
        ...
        CheckError(api->PJRT_Buffer_ToHostBuffer(&bth_args), api, "PJRT_Buffer_ToHostBuffer (output)");
        PJRT_Event* output_transfer_event = bth_args.event;
        ...
        for (size_t i = 0; i < host_output_data.size(); ++i) {
            std::cout << host_output_data[i] << (i == host_output_data.size() - 1 ? "" : ", ");
        }
        std::cout << std::endl;

    } catch (const std::runtime_error& e) {
        std::cerr << "Runtime Error: " << e.what() << std::endl;
        // Note: Resources might not be fully cleaned up here if error occurs mid-setup
    }
    ...
    if (input_buffer) {
        ...
        CheckError(api->PJRT_Buffer_Destroy(&destroy_buf_args), api, "PJRT_Buffer_Destroy (input)");
    ...
    if (output_buffer_from_execute) {
        ...
        CheckError(api->PJRT_Buffer_Destroy(&destroy_buf_args), api, "PJRT_Buffer_Destroy (output)");
    ...
    if (underlying_executable) {
        ...
        destroy_exec_args.struct_size = PJRT_Executable_Destroy_Args_STRUCT_SIZE;
        destroy_exec_args.extension_start = nullptr;
        destroy_exec_args.executable = underlying_executable;
        CheckError(api->PJRT_Executable_Destroy(&destroy_exec_args), api, "PJRT_Executable_Destroy (underlying)");
        std::cout << "Underlying PJRT_Executable object destroyed." << std::endl;
    }
    if (loaded_executable) {
        PJRT_LoadedExecutable_Destroy_Args destroy_loaded_exec_args;
        destroy_loaded_exec_args.struct_size = PJRT_LoadedExecutable_Destroy_Args_STRUCT_SIZE;
        destroy_loaded_exec_args.extension_start = nullptr;
        destroy_loaded_exec_args.executable = loaded_executable;
        CheckError(api->PJRT_LoadedExecutable_Destroy(&destroy_loaded_exec_args), api, "PJRT_LoadedExecutable_Destroy");
        std::cout << "Loaded executable destroyed." << std::endl;
    }
    if (client) {
        ...
        CheckError(api->PJRT_Client_Destroy(&destroy_client_args), api, "PJRT_Client_Destroy");
